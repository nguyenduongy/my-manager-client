...
export default function Organizations () {
    ...
    const [column, setColumn] = useState('organizationName')
    ...
    async function handleSearch () {
        ...
        try {
            const data = await fetchData(`${ENV.domain}/organizations?${newParams}`, requestOptions);
    ...
    useEffect(() => {
        (async () => {
            try {
                const data = await fetchData(`${ENV.domain}/organizations`, requestOptions);
    ...
    async function changePage (page, params) {
        try {
            const data = await fetchData(`${ENV.domain}/organizations?page=${page + params}`, requestOptions);
    ...
    return (
        token ? <div>
            <CCard>
                <CCardBody>
                    <FormControl sx={{left:480,top:10,color:'dodgerblue',fontfamily: "Lucida Console"}}><h1 >Organization Page</h1></FormControl>
                    <CRow>
                    <CCol>
                        ...
                        <Button variant="contained" size="large" onClick={handleClickOpen }>Create New Organization</Button>
                        <Dialog
                    ...
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
                            ...
                            {data && data.map((organization) => (
                                ...
                                >
                                <TableCell component="th" scope="cardID">{organization.organizationID}</TableCell>
                                <TableCell align="right">{organization.organizationID}</TableCell>
                                <TableCell align="right">{organization.organizationName}</TableCell>
                                <TableCell align="right">
                                    <Button onClick={()=>OpendDetails(organization.organizationID)}>Detail</Button>
                                </TableCell>
                                </TableRow>
                            ))}
                            </TableBody>
